...
GPIO_CheckType GPIO_Read(uint8_t GroupId,uint8_t* GroupDataPtr)
{
    ...
    if (( GroupId <GPIO_GROUPS_NUMBER))
    {
        ...
        if ((CfgRead->SetPinType == 0xff)&&(CfgRead->UseAlterFun==0x00)&&(GPIO_GroupState[GroupId] == 1))
            {
...
{
    const GPIO_CfgType * CfgAltFn;
    GPIO_CheckType RetVar;

    if (( GroupId <GPIO_GROUPS_NUMBER))
    {
        CfgAltFn =  & GPIO_ConfigParam[GroupId];

        if ((CfgAltFn->SetPinType == 0xff)&&(CfgAltFn->UseAlterFun==0xff)&&(GPIO_GroupState[GroupId] == 1))
        {

            GPIOPCTL_REG(CfgAltFn->PortId) |= AlternFuncId << (4 * GetPinNumber(CfgAltFn->PortMask));
            RetVar = GPIO_OK ;
...
GPIO_CheckType GPIO_SetInterruptEvent(uint8_t GroupId,GPIO_IntEventType IntEvent,GPIO_IntMaskStatus IntMaskStatus)
{
    ...
    if (( GroupId <GPIO_GROUPS_NUMBER))
    {
        ...
        if ((CfgInt->PortDirection == 0x00)&&(CfgInt->SetPinType == 0xff)&&(CfgInt->UseAlterFun==0x00)&&(GPIO_GroupState[GroupId] == 1))
        {
            if ((0 <= CfgInt->PortId) && (CfgInt->PortId <= 4))
            {
                NVIC_EN0_R |= 1 << CfgInt->PortId;
                RetVar = GPIO_OK ;
            }
            else if (CfgInt->PortId == 0x05)
            {
                NVIC_EN0_R |= 1 << 30;
                RetVar = GPIO_OK ;
            }
            ...
            switch (IntEvent)
            {
                case EVENT_LOW :
                {
                    GPIOIS_REG(CfgInt->PortId) |= 1 << GetPinNumber(CfgInt->PortMask);
                    GPIOIEV_REG(CfgInt->PortId) &= ~ (1 << GetPinNumber(CfgInt->PortMask));
                    RetVar = GPIO_OK ;
                }
                case EVENT_HIGH :
                {
                ...
                {
                    GPIOIS_REG(CfgInt->PortId) &= ~ (1 << GetPinNumber(CfgInt->PortMask));
                    GPIOIBE_REG(CfgInt->PortId) &= ~ (1 << GetPinNumber(CfgInt->PortMask));
                    GPIOIEV_REG(CfgInt->PortId) &= ~ (1 << GetPinNumber(CfgInt->PortMask));
                    RetVar = GPIO_OK ;
                }
            ...
            {
                GPIOIM_REG(CfgInt->PortId) |= 1 << GetPinNumber(CfgInt->PortMask);
            }
            else if (IntMaskStatus == MASK_DISABLED)
            {
                GPIOIM_REG(CfgInt->PortId) &= ~ (1 << GetPinNumber(CfgInt->PortMask));
            }
            else {RetVar = GPIO_NOK ; }
...
GPIO_CheckType GPIO_ClrInterruptFlag(uint8_t GroupId)
{
    ...
    if (( GroupId <GPIO_GROUPS_NUMBER))
    {
        ...
        if ((CfgInt->PortDirection == 0x00)&&(CfgInt->SetPinType == 0xff)&&(CfgInt->UseAlterFun==0x00)&&(GPIO_GroupState[GroupId] == 1))
        {
            GPIOICR_REG(CfgInt->PortId) |= 1 << GetPinNumber(CfgInt->PortMask);
            RetVar = GPIO_OK ;
...
GPIO_CheckType GPIO_GetInterruptStatus(uint8_t GroupId,GPIO_IntStatus *IntStatusPtr)
{
    ...
    if (( GroupId <GPIO_GROUPS_NUMBER))
    {
        ...
        if ((CfgInt->PortDirection == 0x00)&&(CfgInt->SetPinType == 0xff)&&(CfgInt->UseAlterFun==0x00)&&(GPIO_GroupState[GroupId] == 1))
        {
            if ((0 <= CfgInt->PortId) && (CfgInt->PortId <= 4))
            {
                if (((NVIC_ACTIVE0_R >> CfgInt->PortId) & 0x01) == 0x00)
                {
                    * IntStatusPtr = INT_INACTIVE;
                    RetVar = GPIO_OK ;
                }
                else if (((NVIC_ACTIVE0_R >> CfgInt->PortId) & 0x01) == 0x01)
                {
                    if (((NVIC_PEND0_R >> CfgInt->PortId) & 0x01) == 0x01)
                    {
            ...
            {
                if (((NVIC_ACTIVE0_R >> 30) & 0x01) == 0x00)
                {
                    * IntStatusPtr = INT_INACTIVE;
                    RetVar = GPIO_OK ;
                }
                else if (((NVIC_ACTIVE0_R >> 30) & 0x01) == 0x01)
                {
                    if (((NVIC_PEND0_R >> 30) & 0x01) == 0x01)
                    {
                        * IntStatusPtr = INT_PENDING;
                        RetVar = GPIO_OK ;
                    }
...
/*A function to Get a pin number in a port*/
static uint8_t GetPinNumber(uint8_t PinMask)
{
    volatile uint8_t n;
